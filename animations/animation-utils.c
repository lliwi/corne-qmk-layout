...
static void oled_render_anim_frame(const char **fast_frames, const char **slow_frames, uint8_t frames_len) {
  ...
  // Renderizar el primer frame siempre, y después actualizar según el timer
  // Con WPM = 0: mostrar frame estático (idle)
  // Con WPM > 0: animar
  bool should_render = first_render || timer_elapsed32(anim_timer) > ANIM_FRAME_TIME;
  ...
  if (should_render) {
    ...
    // Siempre avanzar el frame para que la animación se mueva
